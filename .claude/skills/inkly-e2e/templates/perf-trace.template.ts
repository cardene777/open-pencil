// Inkly e2e perf-trace template
// 使い方:
//   1. tests/e2e/perf/{name}.spec.ts に copy
//   2. NODE_COUNTS / OUT_LABEL / spec 名を書き換え
//   3. assertion 部分を意図に合わせて変更
//   4. inkly-e2e skill §3 の再現性チェックリストを self-check
...
const OUT_LABEL = 'template' // FIXME spec 名に書き換え
const NODE_COUNTS = [100, 500, 1000, 2000] // FIXME spec の意図に合わせて

const FRAME_MAX_MS = 60 // FIXME spec の閾値に合わせて
...
test.describe(`perf-trace ${OUT_LABEL}`, () => {
  for (const nodeCount of NODE_COUNTS) {
    test(`drag を ${nodeCount} node ドキュメントで計測`, async () => {
      ...
      writeSnapshot(`${OUT_LABEL}-${nodeCount}`, { nodeCount, summary })

      expect(summary?.totalEntries ?? 0).toBeGreaterThan(0)
      const frame = summary?.stats.find((s) => s.name === 'frame')
      expect(frame?.maxMs ?? 0).toBeLessThan(FRAME_MAX_MS)
