...
async function createAndOpenBoard() {
  creating.value = true
  try {
    const board = await createBoard(boardName.value.trim() || boardsT.value.defaultBoardName)
...
<template>
  ...
  >
      ...
      >
        ...
        <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div class="flex w-full max-w-xl flex-col gap-2 md:items-end">
            <AppInput
              v-model="boardName"
              test-id="board-name-input"
              type="text"
              :placeholder="boardsT.defaultBoardName"
            />
            <div class="flex w-full items-center gap-2 md:justify-end">
              <AppInput
                v-model="searchQuery"
                test-id="boards-search"
                type="text"
    ...
    <AlertDialogRoot v-model:open="deleteDialogOpen">
      <AlertDialogPortal>
        <AlertDialogOverlay class="fixed inset-0 z-40 bg-black/55 backdrop-blur-sm" />
        <AlertDialogContent :class="dialogCls.content">
          ...
          <div class="mt-5 flex justify-end gap-2">
            <AlertDialogCancel :class="dialogCls.cancel">
              {{ boardsT.deleteDialogCancel }}
            </AlertDialogCancel>
            <AlertDialogAction
              data-test-id="boards-delete-confirm"
              class="cursor-pointer rounded-lg bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-500/90"
