...
const isExternalUser = computed(() => auth.isExternalUser)
const canCreateBoards = computed(() => !auth.isAuthenticated || !isExternalUser.value)
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <button
            ...
            :disabled="creating || !canCreateBoards"
        ...
        <div
          :class="[
            ...
            isExternalUser ? 'md:grid-cols-4' : 'md:grid-cols-3'
          ]"
        >
          <RouterLink
            to="/boards"
          ...
          <RouterLink
            :to="isExternalUser ? '/account' : '/teams'"
            data-test-id="dashboard-link-teams"
            class="flex flex-col gap-1 rounded-xl border border-white/8 bg-canvas/55 p-4 text-sm text-surface transition-colors hover:bg-hover"
          >
            <icon-lucide-users class="size-5 text-accent" />
            <span class="font-medium">{{ isExternalUser ? 'アカウント' : dashboard.quickActions.teams }}</span>
            <span class="text-xs text-muted">
              {{ isExternalUser ? 'メールアドレスとセッション情報を確認' : dashboard.quickActions.teamsHint }}
            </span>
          </RouterLink>
