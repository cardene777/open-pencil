...
| `text-white` 14px on bg-accent | `#ffffff` | `#3b82f6` | 4.62 (pass) | 4.5 | accent を明るくすると 2.69 (fail) → 両立不能 |
...
- A. 「dark bg 上の text-accent text として 4.5:1 pass」 (明るい青が必要、 `#5b9eff` 以上)
- B. 「accent bg 上の text-white text として 4.5:1 pass」 (暗い青が必要、 `#2563eb` 以下)
両方は満たせない。 解消には accent token を **fg 用 / bg 用に分離** する design system 改修が必要 (例 `--color-accent` + `--color-accent-fg-on-dark`)。
